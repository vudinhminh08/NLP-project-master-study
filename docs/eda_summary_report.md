...
- Tăng so với default 256 trong constants.py — cần kiểm tra trong phase PhoBERT
...
### 7.1 Vấn đề nghiêm trọng cần xử lý ở phase PhoBERT
...
| File | Mô tả | Dùng bởi |
|------|-------|---------|
| `outputs/eda/class_weights.json` | Global + per-aspect weights | phase PhoBERT (weighted loss) |
| `outputs/eda/encoder_config.json` | MAX_SEQ_LEN=384, concat_4_layers | phase PhoBERT (model config) |
...
## 9. Checklist phase PhoBERT

Dựa trên kết quả EDA, cần chú ý khi triển khai phase PhoBERT:
