...
class ABSAPhoBERT(nn.Module):
    ...
    def __init__(
        ...
        self.lambda_sentiment = float(lambda_sentiment)
        ...
        self.register_buffer(
            ...
            torch.full((num_aspects,), self.presence_threshold, dtype=torch.float32),
        )
        ...
        if self.use_attention_pooling:
            self.token_proj = nn.Linear(self.hidden_size, attn_dim)
            self.aspect_queries = nn.Parameter(torch.empty(num_aspects, attn_dim))
    ...
    def get_representations(
        ...
        if not self.use_attention_pooling:
            cls_repr = hidden[:, 0, :]
            return cls_repr.unsqueeze(1).expand(-1, self.num_aspects, -1)

        proj = torch.tanh(self.token_proj(hidden))
        scores = torch.matmul(proj, self.aspect_queries.t())
        mask = (attention_mask == 0).unsqueeze(-1)
        scores = scores.masked_fill(mask, torch.finfo(scores.dtype).min)
        alphas = torch.softmax(scores, dim=1)
        return torch.einsum("bta,bth->bah", alphas, hidden)
    ...
    def get_cls_representation(
        ...
        return self.get_representations(input_ids, attention_mask)[:, 0, :]
    ...
    def forward(
        ...
        aspect_repr = self.get_representations(input_ids, attention_mask)
        ...
        if self.training:
            ...
            presence_logits = [
                torch.stack([all_presence_logits[n][i] for n in range(N_DROPOUT)]).mean(0)
            ...
            for i, logit in enumerate(logits):
                ...
                if self.use_split_loss:
                    ...
                    if class_weights is not None:
                        ...
                        presence_loss = (
                            presence_bce * presence_w
                        ).sum() / presence_w.sum().clamp(min=1e-8)
                    else:
                    ...
                    if present_mask.any():
                        ...
                        log_p_true = sent_log_probs.gather(
                            1, sent_lbl.unsqueeze(1)
                        ).squeeze(1)
